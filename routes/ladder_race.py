# ============================================================
# routes/ladder_race.py
# CS2CaseBot | PvP Ladder Race
#
# Pure winner-take-all race using the exact same LADDER_RUNGS
# fail-chance/multiplier table as solo Ladder Climb and King of the
# Hill Ladder (routes/games_medium.py, routes/koth_ladder.py) -- but
# a completely different money flow. There is no solo-style payout
# here: the entry fee IS the stake, and the first player to safely
# reach the top rung (15) takes the ENTIRE shared pot, no rake,
# matching the existing duel games' no-house-cut PvP convention.
# Busting is a pure loss (no refund). If nobody reaches the top
# within a safety timeout, the round is cancelled and every entry
# fee refunded, since no one legitimately won.
# ============================================================
...
from shared import (
    ...
    secure_random, log_game,
)
from routes.games_medium import LADDER_RUNGS

router = APIRouter(prefix="/api/games/ladder-race", tags=["ladder-race"])

MIN_BET       = 50
MAX_BET       = 750_000
MAX_PLAYERS   = 8
SAFETY_SECS   = 60   # if nobody reaches the top by then, cancel + refund
TOP_RUNG      = len(LADDER_RUNGS)   # 15
...
async def init_ladder_race_tables():
    pool = await get_db()
    async with pool.acquire() as conn:
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS ladder_race_rounds (
        ...
        """)
        await conn.execute("""
            CREATE TABLE IF NOT EXISTS ladder_race_players (
                id            SERIAL PRIMARY KEY,
                round_id      INTEGER NOT NULL REFERENCES ladder_race_rounds(id) ON DELETE CASCADE,
        ...
        """)
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_ladder_race_players_round ON ladder_race_players(round_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_ladder_race_players_user ON ladder_race_players(user_id)")
        await conn.execute("CREATE INDEX IF NOT EXISTS idx_ladder_race_rounds_status ON ladder_race_rounds(status)")
    logger.info("✅ PvP Ladder Race tables ready")


async def recover_stale_ladder_race_rounds():
    pool = await get_db()
    async with pool.acquire() as conn:
        async with conn.transaction():
            stale = await conn.fetch(
                "SELECT id FROM ladder_race_rounds WHERE status='racing' FOR UPDATE"
            )
            for row in stale:
                round_id = row['id']
                players = await conn.fetch(
                    "SELECT user_id, amount FROM ladder_race_players WHERE round_id=$1 AND refunded=FALSE",
                    round_id
                )
                for p in players:
                    await shared.add_balance(p['user_id'], float(p['amount']), conn)
                await conn.execute(
                    "UPDATE ladder_race_players SET refunded=TRUE WHERE round_id=$1", round_id
                )
                await conn.execute(
                    "UPDATE ladder_race_rounds SET status='cancelled', resolved_at=NOW() WHERE id=$1",
                    round_id
                )
            if stale:
                logger.info(f"🏁 Recovered {len(stale)} stale Ladder Race round(s), refunded all entry fees")
...
class LadderRaceRoom:
    def __init__(self, round_id: int):
        self.round_id = round_id
        self.status = 'racing'
        self.pot = 0.0
        # user_id -> {username, amount, rung, busted}
    ...
    async def cancel_if_unresolved(self):
        async with self.lock:
                ...
                async with conn.transaction():
                    for uid, p in self.players.items():
                        if not p['busted']:
                            ...
                            await conn.execute(
                                "UPDATE ladder_race_players SET refunded=TRUE WHERE round_id=$1 AND user_id=$2",
                                self.round_id, uid
                            )
                    await conn.execute(
                        "UPDATE ladder_race_rounds SET status='cancelled', resolved_at=NOW() WHERE id=$1",
            ...
            async with _ladder_race_registry_lock:
                _ladder_race_rooms.pop(self.round_id, None)
        ...
        async with self.lock:
            ...
            async with pool.acquire() as conn:
                async with conn.transaction():
                    ...
                    await conn.execute(
                        "UPDATE ladder_race_rounds SET status='settled', winner_id=$1, resolved_at=NOW() WHERE id=$2",
                        winner_id, self.round_id
                    )
                    for uid, p in self.players.items():
                        await log_game(conn, uid, 'ladder_race', p['amount'],
...
_ladder_race_rooms: Dict[int, LadderRaceRoom] = {}
_ladder_race_registry_lock = asyncio.Lock()


async def _get_or_create_open_room() -> LadderRaceRoom:
    async with _ladder_race_registry_lock:
        for room in _ladder_race_rooms.values():
            if room.status == 'racing' and len(room.players) < MAX_PLAYERS:
                return room
        pool = await get_db()
        async with pool.acquire() as conn:
            row = await conn.fetchrow(
                "INSERT INTO ladder_race_rounds (status) VALUES ('racing') RETURNING id"
            )
        room = LadderRaceRoom(row['id'])
        room.deadline = time.time() + SAFETY_SECS
        _ladder_race_rooms[room.round_id] = room
        return room
...
async def create_private_room(participant_user_ids: list, amount: float) -> int:
    ...
    (_get_or_create_open_room) entirely."""
    if len(participant_user_ids) != 2:
        raise HTTPException(400, "Ladder Race friend challenges need exactly 2 players")
    ...
    async with pool.acquire() as conn:
        async with conn.transaction():
            ...
            for uid in participant_user_ids:
                ...
                await conn.execute(
                    "INSERT INTO ladder_race_players (round_id, user_id, amount) VALUES ($1,$2,$3)",
                    round_id, uid, amount
                )
                players[uid] = {'username': username, 'amount': amount, 'rung': 0, 'busted': False}
            await conn.execute(
                "UPDATE ladder_race_rounds SET pot = $1 WHERE id=$2", amount * len(participant_user_ids), round_id
            )

    room = LadderRaceRoom(round_id)
    room.players = players
    room.pot = round(amount * len(participant_user_ids), 2)
    room.deadline = time.time() + SAFETY_SECS
    async with _ladder_race_registry_lock:
        _ladder_race_rooms[round_id] = room
...
async def join_round(req: JoinRequest, request: Request):
    ...
    await require_game_enabled("ladder-race")
    amount = clamp_bet(req.amount)
    await ensure_user_exists(user_id)

    if req.round_id is not None:
        async with _ladder_race_registry_lock:
            room = _ladder_race_rooms.get(req.round_id)
        if not room:
    ...
    async with room.lock:
        ...
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                await conn.execute(
                    "UPDATE ladder_race_rounds SET pot = pot + $1 WHERE id=$2", amount, room.round_id
                )

        room.pot = round(room.pot + amount, 2)
        room.players[user_id] = {'username': username, 'amount': amount, 'rung': 0, 'busted': False}
...
async def climb(body: RoundIdBody, request: Request):
    user_id = await require_auth(request)
    async with _ladder_race_registry_lock:
        room = _ladder_race_rooms.get(body.round_id)
    if not room:
        raise HTTPException(404, "Round not found or already closed")
    ...
    async with room.lock:
        ...
        fail_chance, _ = LADDER_RUNGS[p['rung']]
        failed = secure_random() < fail_chance
        if failed:
            p['busted'] = True
            async with (await get_db()).acquire() as conn:
                await conn.execute(
                    "UPDATE ladder_race_players SET busted=TRUE WHERE round_id=$1 AND user_id=$2",
            ...
            async with (await get_db()).acquire() as conn:
                await conn.execute(
                    "UPDATE ladder_race_players SET rung=$1 WHERE round_id=$2 AND user_id=$3",
                    p['rung'], room.round_id, user_id
                )
            if p['rung'] >= TOP_RUNG:
                winner_id = user_id

        await room.broadcast({'type': 'player_climbed', 'round_id': room.round_id, 'user_id': user_id,
                               'failed': failed, 'rung': p['rung'], 'busted': p['busted']})
        result = {"success": True, "failed": failed, "rung": p['rung'], "busted": p['busted'], "won": winner_id is not None}
...
async def list_rounds():
    async with _ladder_race_registry_lock:
        rooms = list(_ladder_race_rooms.values())
    result = {"rounds": [
        {
...
async def get_round(round_id: int):
    async with _ladder_race_registry_lock:
        room = _ladder_race_rooms.get(round_id)
    if room:
        return convert_decimals(room.snapshot())

    pool = await get_db()
    async with pool.acquire() as conn:
        rnd = await conn.fetchrow("SELECT * FROM ladder_race_rounds WHERE id=$1", round_id)
        if not rnd:
            raise HTTPException(404, "Round not found")
        players = await conn.fetch("""
            SELECT p.*, u.username FROM ladder_race_players p
...
async def ladder_race_ws(websocket: WebSocket, round_id: int):
